...
final class DataCenter {
    ...
    func createTriviaQuiz(
        ...
        let request = URLRequest(url: query.apiURL!)
        return try await makeAPICall(
            with: request,
            processResponse: { (response: OpenTriviaDB.APIResponse) in
                if response.code == 1 {
    ...
    /// - Parameter processResponse: The closure to process the raw API
    ///                              response body.
    ///
